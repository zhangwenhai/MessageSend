...
import com.theone.sns.component.upload.rs.ChunkUploadCallRet;
import com.theone.sns.component.upload.rs.PutExtra;
import com.theone.sns.component.upload.utils.Crc32;
import com.theone.sns.component.upload.utils.InputStreamAt;
import com.theone.sns.component.upload.utils.InputStreamAt.Input;
...
public class SliceUploadTask extends UploadTask {
	...
	private CallRet uploadBlocks(List<Block> blks) throws IOException {
		...
		for (int i = 0; i < blkCount; i++) {
			...
			if (blk != null) {
				...
				ChunkUploadCallRet chunkRet = upBlock(l, data,
						Conf.CHUNK_TRY_TIMES);
				...
				Block nblk = new Block(i, chunkRet.getCtx(), l,
						chunkRet.getHost());
	...
	private ChunkUploadCallRet upBlock(int len, Input data, int time)
			throws IOException {
	...
	private String buildMkfileUrl(String lastHost) {
		...
		StringBuilder url = new StringBuilder(lastHost + "/mkfile/"
				+ contentLength);
	...
	static class UpBlock {
		...
		ChunkUploadCallRet uploadChunk() throws IOException {
			...
			if (length > FIRST_CHUNK) {
				...
				for (int i = 0; i < count; i++) {
					...
					if (task.isCancelled()) {
						return new ChunkUploadCallRet(Conf.CANCEL_CODE, "",
								Conf.PROCESS_MSG);
					}
		...
		private ChunkUploadCallRet uploadMkblk(long blkLength,
				byte[] chunkData, int time) {
			...
			if (!ret.isOk()) {
				...
				if (Util.needChangeUpAdress(ret)) {
					orginHost = Conf.UP_HOST.equals(orginHost) ? Conf.UP_HOST2
							: Conf.UP_HOST;
				}
		...
		private ChunkUploadCallRet uploadChunk(ChunkUploadCallRet ret,
				byte[] chunkData) {
		...
		private ChunkUploadCallRet upload(String url, byte[] chunkData, int time) {
			...
			try {
				...
				task.post.setHeader("Authorization",
						"UpToken " + task.auth.getUploadToken());

				task.post.setEntity(buildHttpEntity(chunkData));

				HttpResponse response = task.getHttpClient().execute(task.post);

				ChunkUploadCallRet ret = new ChunkUploadCallRet(
				...
				int status = task.isCancelled() ? Conf.CANCEL_CODE
						: Conf.ERROR_CODE;
		...
		private ChunkUploadCallRet checkAndRetryUpload(String url,
				byte[] chunkData, int time, ChunkUploadCallRet ret)
				throws IOException {
			...
			if (!ret.isOk()) {
				...
				if (ret.getCrc32() != crc32) {
					...
					if (time > 0) {
						...
						// 406 上传的数据 CRC32 校验错。
						return new ChunkUploadCallRet(Conf.ERROR_CODE, "",
								"local's crc32 do not match.");
					}
				} else {
		...
		private boolean needPutRetry(ChunkUploadCallRet ret) {
			...
			return ret.getStatusCode() == 406 || ret.getStatusCode() == 996
					|| ret.getStatusCode() / 100 == 5;
		}
		...
		private String getBlkUrl(ChunkUploadCallRet ret) {
			...
			String url = ret.getHost() + "/bput/" + ret.getCtx() + "/"
					+ ret.getOffset();
