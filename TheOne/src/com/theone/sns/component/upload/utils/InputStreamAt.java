...
public abstract class InputStreamAt implements Closeable {
	...
	public static FileInput fromInputStream(Context context, InputStream is)
			throws IOException {
	...
	public static class FileInput extends InputStreamAt {
		...
		public FileInput(File file, String aliasFilename)
			...
			this.filename = (aliasFilename != null && aliasFilename.trim()
					.length() > 0) ? aliasFilename : file.getName();
		}
	...
	public static class UriInput extends InputStreamAt {
		...
		public long crc32() throws IOException {
			...
			if (fileInput != null) {
				...
				UriInfo info = new UriInfo(uriInfo.getContext(),
						uriInfo.getUri());
	...
	public static class UriInfo {
		...
		/**
		 * 通过uri查找文件，若找到，构建基于文件的 InputStreamIsa ，委托七处理; 否则获取流
		 *
		 * @throws FileNotFoundException
		 */
		...
		private void checkContent() {
			...
			if ("content".equalsIgnoreCase(uri.getScheme())) {
				...
				try {
					...
					String[] col = { MediaStore.MediaColumns.SIZE,
					...
					if (cursor != null && cursor.moveToFirst()) {
						...
						for (int i = 0; i < cc; i++) {
							...
							if (MediaStore.MediaColumns.DISPLAY_NAME
									.equalsIgnoreCase(colName)) {

								name = colValue;

							} else if (MediaStore.MediaColumns.SIZE
									.equalsIgnoreCase(colName)) {

								length = cursor.getLong(i);

							} else if (MediaStore.MediaColumns.MIME_TYPE
									.equalsIgnoreCase(colName)) {

								mimeType = colValue;

							} else if (MediaStore.MediaColumns.DATA
									.equalsIgnoreCase(colName)) {
