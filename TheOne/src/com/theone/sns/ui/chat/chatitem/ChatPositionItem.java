 ...
 */
public class ChatPositionItem extends ChatBaseItem {

    private final IChatLogic mIChatLogic;

    protected ChatPositionItem(MessageInfo mMessageInfom, IChatLogic mIChatLogic) {
    ...
    public View layoutInflater(LayoutInflater inflater) {
        if (FusionConfig.getInstance().getUserId().equals(mMessageInfo.owner.userId)) {
            return inflater.inflate(R.layout.chat_location_send, null);
        } else {
            return inflater.inflate(R.layout.chat_location_recv, null);
        }
    }
    ...
    public void bindView(View view, ImageLoaderViewHolder holder) {
        ...
        holder.mTextView = (TextView) view.findViewById(R.id.location_text);
    ...
    public void drowView(final ImageLoaderViewHolder holder,
        ...
        if (null == mMessageInfo.position) {
            return;
        }

        holder.mTextView.setText(mMessageInfo.position.name);

        holder.imageView1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (null == mMessageInfo.position.location
                        || mMessageInfo.position.location.size() != 2) {
                    return;
                }
                mActivity.startActivity(new Intent(FusionAction.ChatAction.SHOW_LOCATION_ACTION)
                        .putExtra(FusionAction.ChatAction.LONGITUDE,
                                mMessageInfo.position.location.get(0)).putExtra(
                                FusionAction.ChatAction.LATITUDE,
                                mMessageInfo.position.location.get(1)));
            }
        });

        holder.imageView1.setImageDrawable(mActivity.getResources().getDrawable(R.drawable.ditu));
