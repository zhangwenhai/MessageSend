package com.theone.sns.util.uiwidget.pulltorefresh;
...
import com.theone.sns.util.uiwidget.pulltorefresh.internal.LoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel,
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel,
					pullLabel, refreshingLabel);
		...
		if (a.hasValue(R.styleable.PullToRefresh_headerBackground)) {
			this.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_headerBackground,
					Color.WHITE));
		}
		if (a.hasValue(R.styleable.PullToRefresh_adapterViewBackground)) {
			refreshableView.setBackgroundResource(a.getResourceId(
					R.styleable.PullToRefresh_adapterViewBackground, Color.WHITE));
		}
		a.recycle();
	...
	private boolean pullEvent() {
		...
		if (headerHeight * 1.2 >= Math.abs(newHeight)) {
			setHeaderScroll(newHeight);
		} else {
			if (newHeight > 0) {
				newHeight = (int) (headerHeight * 1.2);
			} else {
				newHeight = -(int) (headerHeight * 1.2);
			}
			setHeaderScroll(newHeight);
